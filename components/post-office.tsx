...
export function PostOffice({ user, onNavigate }: PostOfficeProps) {
  ...
  const handleSelectLetter = (letter: UnsentLetter) => {
  ...
  const handleSend = (option: "now" | "scheduled") => {
  ...
  return (
    <div className="min-h-screen relative">
      {/* Background - Post Office Interior */}
      <div className="absolute inset-0 bg-gradient-to-br from-amber-100 to-amber-200"></div>

      {/* Wooden Counter Surface */}
      <div className="absolute bottom-0 left-0 right-0 h-1/4 bg-gradient-to-t from-amber-800 to-amber-700 opacity-40"></div>

      {/* Back Button */}
      <Button
        onClick={() => (step === "select" ? onNavigate("main") : setStep("select"))}
        className="absolute top-6 left-6 z-10 bg-amber-700 hover:bg-amber-600 text-amber-100"
      >
      ...
      <div className="flex h-screen items-center justify-center p-6">
        <div className="relative w-full max-w-5xl">
          ...
          {step === "select" && (
            <div className="bg-white rounded-lg shadow-2xl p-8">
              <div className="text-center mb-8">
                ...
                <h2 className="text-3xl font-serif text-amber-800 mb-4">Digital Post Office</h2>
                <p className="text-amber-700 font-serif text-lg">Select a letter to send</p>
              </div>

              {unsentLetters.length > 0 ? (
                <div className="space-y-4">
                  <h3 className="text-xl font-serif text-amber-800 mb-6 text-center">Your Unsent Letters</h3>
                  {unsentLetters.map((letter) => (
                    <div
                      key={letter.id}
                      className="bg-amber-50 border border-amber-200 rounded-lg p-6 hover:bg-amber-100 hover:shadow-lg transition-all duration-300 cursor-pointer"
                      onClick={() => handleSelectLetter(letter)}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          {/* Letter Preview */}
                          <div className="w-16 h-12 bg-white rounded shadow-md border border-gray-200 relative">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center mb-2">
                              ...
                              <h4 className="font-serif text-amber-800 font-bold text-lg">{letter.recipient}</h4>
                            </div>
                            <p className="text-amber-700 font-serif mb-1">{letter.subject}</p>
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-green-700 mb-2">${letter.estimatedCost}</div>
                          <Button className="bg-amber-600 hover:bg-amber-700 text-white">Select Letter</Button>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-serif text-gray-600 mb-4">No Unsent Letters</h3>
                  <p className="text-gray-500 mb-6">You don't have any letters ready to send.</p>
                  <Button onClick={() => onNavigate("personal")} className="bg-amber-600 hover:bg-amber-700 text-white">
                    Write a New Letter
                  </Button>
                </div>
              )}
          ...
          {step === "counter" && selectedLetter && (
            <div className="text-center">
              ...
              <div className="relative mx-auto mb-8">
                <div className="w-80 h-52 bg-gradient-to-br from-white to-amber-50 rounded-lg shadow-2xl border border-amber-200 relative overflow-hidden">
                  {/* Letter Content Simulation */}
                  <div className="absolute inset-6 space-y-3">
                    <div className="h-1 bg-blue-200 opacity-60 rounded"></div>
                    <div className="h-1 bg-blue-200 opacity-60 rounded w-4/5"></div>
                    <div className="h-1 bg-blue-200 opacity-60 rounded w-3/4"></div>
                    <div className="h-1 bg-blue-200 opacity-60 rounded w-5/6"></div>
                  </div>

                  {/* Recipient Address Area */}
                  <div className="absolute bottom-4 right-4 bg-amber-100 p-3 rounded border border-amber-300">
                  ...
                  {/* Seal */}
                  <div className="absolute top-4 right-4 w-8 h-8 bg-red-600 rounded-full opacity-90 flex items-center justify-center">
                    <div className="w-6 h-6 border-2 border-red-200 rounded-full"></div>
                  </div>
                </div>

                {/* Letter Shadow */}
                <div className="absolute -bottom-2 left-8 right-8 h-4 bg-black opacity-10 blur-lg rounded-full"></div>
              </div>

              <div className="bg-white rounded-lg shadow-lg p-8 max-w-md mx-auto">
                <h2 className="text-2xl font-serif text-amber-800 mb-4">Post Office Counter</h2>
                ...
                <Button
                  onClick={handleStamp}
                  className="bg-amber-700 hover:bg-amber-600 text-amber-100 text-lg px-8 py-4 w-full"
                >
          ...
          {step === "stamping" && selectedLetter && (
            <div className="text-center">
              <div className="bg-white rounded-lg shadow-2xl p-8 max-w-lg mx-auto">
                <h2 className="text-2xl font-serif text-amber-800 mb-6">Authentication in Progress</h2>

                {/* Stamping Animation */}
                <div className="relative mx-auto mb-8">
                  <div className="w-64 h-40 bg-gradient-to-br from-white to-amber-50 rounded-lg shadow-lg border border-amber-200 relative overflow-hidden">
                    {/* Stamp Effect with Animation */}
                    <div
                      className={`absolute top-4 right-4 w-16 h-16 bg-red-600 rounded-lg transition-all duration-300 ${
                        stampingProgress > 50 ? "opacity-90 scale-100" : "opacity-60 scale-95"
                      }`}
                    >
                      <div className="absolute inset-2 border-2 border-red-200 rounded flex items-center justify-center">
                        <div className="text-center">
                    ...
                    {stampingProgress > 30 && (
                      <div className="absolute top-4 left-4 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center animate-pulse">
                        <CheckCircle className="w-4 h-4 text-white" />
                      </div>
                    )}

                    {stampingProgress > 60 && (
                      <div className="absolute bottom-4 left-4 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center animate-pulse">
                        <CheckCircle className="w-4 h-4 text-white" />
                      </div>
                    )}
                  </div>
                  ...
                  {/* Stamp Machine */}
                  <div
                    className={`absolute -top-12 right-16 w-20 h-24 bg-gradient-to-b from-gray-700 to-gray-900 rounded-lg shadow-xl transition-transform duration-200 ${
                      stampingProgress > 20 && stampingProgress < 80 ? "transform translate-y-2" : ""
                    }`}
                  >
                    <div className="absolute bottom-0 w-full h-6 bg-red-600 rounded-b-lg"></div>
                ...
                <div className="space-y-2 text-sm">
                  {stampingProgress > 20 && <p className="text-green-600 font-serif">✓ Letter authenticity verified</p>}
          ...
          {step === "payment" && selectedLetter && (
            <div className="text-center">
              <div className="bg-white rounded-lg shadow-2xl p-8 max-w-lg mx-auto">
                <h2 className="text-2xl font-serif text-amber-800 mb-6">Payment</h2>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <p className="font-serif text-amber-800">Credit Card</p>
                    </button>
                    ...
                    >
                      ...
                      <p className="font-serif text-amber-800">Cash</p>
                    </button>
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white text-lg px-8 py-3 w-full disabled:opacity-50"
                >
                  Complete Payment
                </Button>
              </div>
          ...
          {step === "sending" && selectedLetter && !showScheduler && (
            <div className="text-center">
              <div className="bg-white rounded-lg shadow-2xl p-8 max-w-lg mx-auto">
                <h2 className="text-2xl font-serif text-amber-800 mb-8">Choose Delivery Method</h2>

                <div className="grid grid-cols-2 gap-8">
                  {/* Send Now */}
                  <div className="text-center">
                    <div
                      className={`w-32 h-48 bg-gradient-to-b from-gray-600 to-gray-800 rounded-lg shadow-2xl cursor-pointer transition-all duration-300 mx-auto mb-4 relative ${
                        sendOption === "now" ? "transform scale-110 shadow-xl" : "hover:scale-105"
                      }`}
                      onClick={() => handleSend("now")}
                    >
                      {/* Lever Handle */}
                      <div className="absolute top-6 left-1/2 transform -translate-x-1/2 w-16 h-8 bg-red-600 rounded-full shadow-lg"></div>

                      {/* Lever Body */}
                      <div className="absolute top-14 left-1/2 transform -translate-x-1/2 w-2 h-20 bg-gray-400 rounded"></div>
                      ...
                      {/* Base */}
                      <div className="absolute bottom-0 left-0 right-0 h-8 bg-gray-900 rounded-b-lg"></div>
                    </div>
                    <h3 className="font-serif text-amber-800 font-bold">SEND NOW</h3>
                    <p className="text-sm text-amber-600">Immediate delivery</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    >
                      {/* Lever Handle */}
                      <div className="absolute top-6 left-1/2 transform -translate-x-1/2 w-16 h-8 bg-blue-600 rounded-full shadow-lg"></div>
                    ...
                    <h3 className="font-serif text-amber-800 font-bold">SCHEDULE</h3>
                    <p className="text-sm text-amber-600">Choose delivery time</p>
                  </div>
                </div>
            ...
            <div className="text-center">
              <div className="bg-white rounded-lg shadow-2xl p-8 max-w-lg mx-auto">
                <div className="mb-6">
                  ...
                  <h2 className="text-2xl font-serif text-amber-800 mb-2">Schedule Delivery</h2>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-amber-50 p-6 rounded-lg border border-amber-200">
                    <h3 className="font-serif text-amber-800 font-bold mb-4 flex items-center">📅 Select Date</h3>
                    <input
                      type="date"
                      value={scheduledDate}
                  ...
                  <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
                    <h3 className="font-serif text-amber-800 font-bold mb-4 flex items-center">🕐 Select Time</h3>
                    <input
                      type="time"
                      value={scheduledTime}
